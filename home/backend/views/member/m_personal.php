...
elseif($act == 'set_account_save')
{
	...
	{
		$re_u['email']=$setsqlarr['email'];
		$mem_info_u['email']=$setsqlarr['email'];
	
		$re_u['telephone']=$setsqlarr['mobile'];
		$mem_info_u['phone']=$setsqlarr['mobile'];
		
		$db->updatetable(table('resume'),$re_u," uid={$thisuid}");
		$db->updatetable(table("members_info"),$mem_info_u,array("uid"=>intval($thisuid)));
		//填写管理员日志
